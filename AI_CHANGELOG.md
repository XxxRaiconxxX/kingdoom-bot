...
*   **Cambios Clave:**
    ...
    *   **[Soporte !data Extendido & Descarga Resiliente Multi-Vía]:** Rediseñado el comando `!data` en `src/handlers/admin.js` para soportar 3 vías independientes de carga de conocimiento: (1) descarga de adjuntos `.txt` con reintentos y fallback a extracción directa de `_blob` en memoria via Puppeteer context (`pupPage.evaluate`), (2) lectura de contenido de mensajes citados de texto o archivos, y (3) carga directa de texto multilínea en el cuerpo del mensaje `!data <titulo>\n\n<contenido>`. Esto garantiza 100% de éxito en la asimilación de conocimiento sin importar restricciones de media en WhatsApp.
