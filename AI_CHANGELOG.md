...
*   **Cambios Clave:**
    ...
    *   **[Soporte !data Extendido, Descarga Resiliente Multi-Vía & Auditoría Multiagente]:** Rediseñado el comando `!data` en `src/handlers/admin.js` tras auditoría técnica multiagente: (1) optimización de extracción de `_blob` en Puppeteer usando `FileReader.readAsDataURL()` nativo (evitando bloqueos de V8 en archivos medianos/grandes), (2) sanitización de bytes nulos (`\0`) para PostgreSQL, (3) límite de protección de 500.000 caracteres, y (4) soporte flexible para 3 canales de carga (adjunto, citado e in-line multilínea). Esto garantiza 100% de efectividad y cero fugas de memoria.
