...
*   **Cambios Clave:**
    ...
    *   **[Soporte !data Extendido & Descarga Resiliente]:** Mejorado el comando `!data` en `src/handlers/admin.js` para permitir la carga de archivos `.txt` tanto adjuntos directamente como mediante respuesta (*quoted message*), agregando un mecanismo de reintento automático (hasta 3 intentos con 600ms de espera) y re-obtención del mensaje completo por ID (`getMessageById`) si WhatsApp tarda en inicializar las llaves de desencriptación del adjunto. Soporta MIME types como `application/octet-stream` y ofrece mensajes de diagnóstico detallados.
