...
*   **Cambios Clave:**
    ...
    *   **[Soporte !data Extendido, Resolución Sintética de Citas & Fix de Captions]:** Implementada resolución sintética directa desde `msg._data.quotedMsg` para mensajes citados cuando `client.getMessageById` arroja la excepción minificada `r`. Rediseñado el pipeline de descarga de documentos `.txt` en `src/handlers/admin.js` para usar `activePupPage` de forma prioritaria con 12s de polling activo. Pasó el 100% de la suite de pruebas.
