...
*   **Cambios Clave:**
    ...
    *   **[Soporte !data Extendido, Fix de Captions en Documentos & Descarga MMS Resiliente]:** Corregida la causa raíz en `src/index.js` y `src/handlers/admin.js`: WhatsApp Web asigna el texto de los comandos en archivos adjuntos a la propiedad `msg.caption` en lugar de `msg.body` (que contiene solo el nombre del archivo o viene vacío). Se actualizó la extracción de comandos para priorizar `msg.caption`, permitiendo que `!data <titulo>` ejecutado como pie de foto/documento se parsee y procese correctamente. Se agregó además un guard para ignorar respuestas del bot al citar mensajes.
