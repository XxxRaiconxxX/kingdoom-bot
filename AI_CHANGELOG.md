...
*   **Resumen de Tareas:** Optimizacion del runtime Ubuntu para una laptop HP 15-ra0xx con Intel Celeron N3060, 2 cores y 4 GB RAM. Docker Compose ahora usa `init: true`, `stop_grace_period: 30s`, `mem_limit: 1280m`, `mem_reservation: 512m`, `shm_size: 256m`, `cpus: 1.8`, `pids_limit: 256` y healthcheck HTTP contra `/healthz`. Se probo quitar `--disable-dev-shm-usage`, pero se revirtio porque el primer arranque quedo demasiado tiempo en `CONNECTING`; se conserva el flag estable y el `/dev/shm` ampliado como margen seguro del contenedor.
*   **Validacion:** `docker compose config --quiet`, `node --check src/index.js`, `git diff --check` y seis pruebas dirigidas (`CONNECTION_WATCHDOG_OK`, `WHATSAPP_HEALTH_OK`, `WHATSAPP_RECOVERY_OK`, `PROCESS_SUPERVISOR_OK`, `ROLEPLAY_ACTIVITY_OK`, `ROLEPLAY_PERSISTENCE_OK`) superados. Rebuild controlado completado; `/status` reporta `operational=true`, `connectionHealth=HEALTHY`, 23 probes funcionales consecutivos sin fallos y `/healthz` devuelve HTTP 200.
*   **Riesgos / Advertencias:** Estos limites priorizan tolerar picos del bot en una laptop dedicada, dejando algo de CPU al host y evitando explosion de procesos. Si WhatsApp Web vuelve a acercarse al limite de memoria, revisar `/status`, `docker stats`, PIDs y logs antes de subir mas limites.
