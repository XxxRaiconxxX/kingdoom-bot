...
*   **Cambios Clave:**
    ...
    *   **[Soporte !data Extendido, Polling de MediaStage & Descarga MMS Resiliente]:** Corregido fallo de desincronización en descargas de documentos `.txt` en WhatsApp Web Headless: al recibir adjuntos en segundo plano, WhatsApp asigna `mediaStage = NOT_LOADED`. Se implementó un bucle de polling asíncrono de hasta 8 segundos dentro de Puppeteer (`pupPage.evaluate`) para solicitar la descarga de MMS y esperar activamente la transición a `mediaStage = RESOLVED` antes de leer el `_blob`. Se mejoró además la resolución del ID de mensajes envueltos (`_originalMsg`). Esto garantiza la asimilación exitosa de archivos adjuntos `.txt` de cualquier tamaño.
