...
*   **Cambios Clave:**
    ...
    *   **[Soporte !data Extendido, Descarga Directa Puppeteer-First & Fix de Captions]:** Rediseñado el pipeline de descarga de documentos `.txt` en `src/handlers/admin.js`: la descarga ejecuta directamente la evaluación en Puppeteer (`pupPage.evaluate`) con polling activo de `mediaStage = RESOLVED` de hasta 12 segundos, evitando el fallo nativo de `FETCHING` en `whatsapp-web.js`. Sincronizada la lectura de `msg.caption` en `src/index.js` para comandos en archivos adjuntos. Pasó el 100% de la suite de pruebas.
