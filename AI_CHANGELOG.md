...
*   **Cambios Clave:**
    ...
    *   **[Soporte !data Extendido, Patch de la Comunidad & Descarga MMS Resiliente]:** Integrado el parche oficial de la comunidad para variaciones de módulos en `WAWebDownloadManager` (`downloadAndMaybeDecrypt` || `downloadAndDecrypt` || `downloadMedia`). Corregida la extracción de comandos con pie de foto/documento (`msg.caption`), adicionando polling activo de `mediaStage = RESOLVED` y 3 vías independientes de carga. Esto garantiza compatibilidad 100% con los últimos cambios internos de WhatsApp Web.
