...
export function normalizePhone(phone) {
  ...
  // 4. Map owner 15-digit ID to main phone number so the same profile is used (via env)
  const envOwnerAlt = process.env.OWNER_ALT_ID ? normalizePhone(process.env.OWNER_ALT_ID) : null;
  const envOwner = process.env.OWNER_NUMBER ? normalizePhone(process.env.OWNER_NUMBER) : null;

  if (envOwnerAlt && envOwner && cleaned === envOwnerAlt) {
    cleaned = envOwner;
  }
