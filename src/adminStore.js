...
export function isOwner(whatsappNumber) {
  ...
  const envOwner = typeof process.env.OWNER_NUMBER !== 'undefined' && process.env.OWNER_NUMBER ? normalizePhone(process.env.OWNER_NUMBER) : null;
  const envAdmin = typeof process.env.ADMIN_NUMBER !== 'undefined' && process.env.ADMIN_NUMBER ? normalizePhone(process.env.ADMIN_NUMBER) : null;
