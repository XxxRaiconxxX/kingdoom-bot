...
export function isOwner(whatsappNumber) {
  ...
  const envOwner = process.env.OWNER_NUMBER !== undefined ? normalizePhone(process.env.OWNER_NUMBER) : null;
  const envAdmin = process.env.ADMIN_NUMBER !== undefined ? normalizePhone(process.env.ADMIN_NUMBER) : null;
  
  return (envOwner !== null && phone === envOwner) ||
         (envAdmin !== null && phone === envAdmin);
}
