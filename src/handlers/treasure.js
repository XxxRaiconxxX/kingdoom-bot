...
export async function dropTreasure(client) {
  try {
    ...
    const targetGroup = process.env.TREASURE_GROUP_ID;
    if (!targetGroup) {
      console.warn('[Treasure] process.env.TREASURE_GROUP_ID no esta configurado. Omitiendo.');
      return null;
    }
...
export function scheduleDailyTreasures(client) {
  ...
  const numEvents = secureRandomInt(2, 4);
